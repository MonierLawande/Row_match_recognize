...
class ConditionEvaluator(ast.NodeVisitor):
    ...
    def visit_Name(self, node: ast.Name):
        ...
        if re.match(r'^[A-Za-z_][A-Za-z0-9_]*$', node.id):
            ...
            # Temporarily disable debug logging to avoid recursion
        ...
        if isinstance(node.func, ast.Name):
            ...
            if func_name in ("PREV", "NEXT", "FIRST", "LAST"):
                ...
                if is_nested:
                    ...
                    if func_name == "PREV":
                        # Handle PREV with variable-assignment-aware logic for DEFINE clauses
                        if len(args) >= 2:
                            # PREV(B.value) format - variable-assignment-aware
                            var_name = args[0]
                            column = args[1]
                            steps = args[2] if len(args) > 2 else 1
                            return self._get_navigation_value(var_name, column, 'PREV', steps)
                        else:
                        ...
                        # Handle NEXT with variable-assignment-aware logic for DEFINE clauses
                        if len(args) >= 2:
                            # NEXT(B.value) format - variable-assignment-aware
                            var_name = args[0]
                            column = args[1]
                            steps = args[2] if len(args) > 2 else 1
                            return self._get_navigation_value(var_name, column, 'NEXT', steps)
                        else:
...
def compile_condition(condition_expr, row_context=None, current_row_idx=None, current_var=None):
    ...
    # print(f"CONDITION_DEBUG: Original='{condition_expr}' -> Processed='{processed_condition}'")
    ...
    if row_context is None and current_row_idx is None:
        ...
        def condition_fn(row, context):
            ...
            # Don't perform expensive linear search - current_idx is already set by the matcher
