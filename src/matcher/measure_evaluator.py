...
class MeasureEvaluator:
    ...
    def evaluate(self, expr: str) -> Any:
        ...
        if var_match:
            ...
            # Get the appropriate row based on running/final semantics
            if var in self.context.variables:
                indices = self.context.variables[var]
                if indices:
                    if is_running:
                        # Get the last row up to current position
                        valid_indices = [idx for idx in indices if idx <= self.context.current_idx]
                        if valid_indices:
                            return self.context.rows[valid_indices[-1]].get(col)
                    else:
                        # Get the last row overall
                        return self.context.rows[indices[-1]].get(col)
            return None
    ...
    def _evaluate_navigation(self, expr: str, is_running: bool) -> Any:
        ...
        if func_name in ("FIRST", "LAST"):
            ...
            if var_match:
                ...
                if var in self.context.variables:
                    indices = sorted(self.context.variables[var])
                    if indices:
                        if func_name == "FIRST":
                            if occurrence < len(indices):
                                return self.context.rows[indices[occurrence]].get(col)
                        else:  # LAST
                            if occurrence < len(indices):
