% ============================================================
%  ch5_experimental_evaluation.tex
...
implementation across three dimensions: output correctness (parity
with Trino), semantic coverage (SQL:2016 feature completeness), and
developer effort (reduction in lines of code compared with
hand-written Pandas idioms).  Section~\ref{sec:eval-methodology}
...
We evaluate the proposed \texttt{MATCH\_RECOGNIZE} implementation
through three complementary aspects.

\begin{enumerate}
  \item \textbf{Output correctness.}
    Results are compared row-by-row with Trino across multiple
    pattern types to ensure identical matched rows, pattern
    alignment, and measure calculations.

  \item \textbf{Semantic coverage.}
    Compliance with SQL:2016 \texttt{MATCH\_RECOGNIZE} semantics is
    verified, covering \texttt{PERMUTE} patterns, exclusions,
    quantifiers, navigation functions, and output
    modes~\cite{iso2016,ISO2021,monierashraf2025rowmatch}.

  \item \textbf{Developer effort (code size).}
    The lines/tokens required to express equivalent logic in
    idiomatic Pandas serve as a baseline.  Hand-written Pandas
    solutions typically require substantially more code (e.g.\
    chained \texttt{groupby}/\texttt{shift}/joins) than a single
    \texttt{MATCH\_RECOGNIZE} statement, so reducing LOC is a core
    goal of the row-pattern recognition (RPR)
...
Each test case is assessed on the three dimensions defined above.
For \emph{output correctness}, the result DataFrame produced by the
implementation is compared with the result set returned by Trino
using an exact row-level equality check (same rows, same column
values, same order where order is deterministic).  For
\emph{semantic coverage}, each SQL:2016 feature is marked as
\emph{supported}, \emph{partially supported}, or
\emph{unsupported} based on whether the test suite exercises the
feature and the output matches Trino.  For \emph{developer effort},
the number of non-blank, non-comment lines in the equivalent
idiomatic Pandas solution is recorded and compared with the
single-statement \texttt{MATCH\_RECOGNIZE} query.
...
We use an \texttt{orders} table to demonstrate and validate
...
This structure enables comprehensive validation of partitioning,
ordering, and pattern detection (e.g.\ sequences of rising or
falling prices per customer).  In the experiments, data are loaded
via a \texttt{VALUES} clause in Trino and verified to produce
exactly the same matched rows and measures as the Pandas
implementation.
...
three independent approaches---our proposed engine, Trino~473, and a
...
The experiments were conducted on a workstation running Ubuntu~24.04.2~LTS,
equipped with an Intel Core i7-11800H processor (16~cores at 2.30\,GHz)
and 16\,GB of DDR4 RAM at 3200\,MHz.  The baseline SQL engine was
Trino~(version~473).  The software environment included Java~11.0.25~LTS
and Python~3.12.7, together with Python libraries such as
\texttt{pandas}~2.2.3, \texttt{numpy}~1.26.4,
\texttt{antlr4-python3-runtime}~4.13.2, \texttt{antlr4-tools}~0.2.1,
\texttt{pytest}~8.3.4, Jupyter, \texttt{unittest},
\texttt{matplotlib}~3.10.0, \texttt{plotly}~5.24.1, and
\texttt{seaborn}~0.13.2.  Standard Python library modules used
include \texttt{typing}, \texttt{dataclasses}, \texttt{re},
\texttt{operator}, \texttt{math}, \texttt{itertools},
\texttt{collections}, \texttt{enum}, \texttt{time}, \texttt{sys},
\texttt{os}, \texttt{logging}, and \texttt{traceback}.
...
We use the \texttt{stock\_price} table to demonstrate and validate
...
This example detects bullish momentum by identifying three consecutive
price increases $(A < B < C)$.  The query partitions records by company
(\texttt{PARTITION BY company}), orders them chronologically
(\texttt{ORDER BY price\_date}), and applies \texttt{PATTERN (A B C)}
with a \texttt{DEFINE} condition enforcing rising prices.  Key
attributes---starting price, ending price, and match ID---are produced
in the \texttt{MEASURES} clause.  A structured \texttt{pandas} DataFrame
serves as input to \texttt{MATCH\_RECOGNIZE}, enabling automated
detection of upward trends across firms under SQL:2016 semantics.
...
In the second step, we applied the same pattern-matching query using the
same data in the Trino environment to validate the correctness of our
...
consistent with the Pandas DataFrame used earlier.  We then executed
the \texttt{MATCH\_RECOGNIZE} query in Trino to detect three consecutive
price increases per company in accordance with SQL:2016 semantics.  In
addition, we validated the same query in Oracle Database via
...
rows; the Oracle results matched the expected behaviour, strengthening
...
As a Python baseline, we implemented the same pattern logic directly in
...
and sorted by company and trading date.  For each company, we compute a
Boolean signal \texttt{inc\,:=\,diff(Price)\,>\,0} and mark candidate
starts where \texttt{inc}, \texttt{inc.shift(-1)}, and
\texttt{inc.shift(-2)} are all true, corresponding to a three-row
uptrend ($A < B < C$).  Candidates are processed sequentially with a
...
All three approaches---our proposed engine, Trino~473, and the
hand-written Pandas baseline---produce identical output, confirming
the correctness of the implementation under SQL:2016 semantics.
...
[Present throughput and scalability results here.]
...
[Compare with Oracle, Trino, Flink, etc.]
...
Our \texttt{MATCH\_RECOGNIZE} implementation supports a wide range of
...
To assess correctness, we executed each pattern-matching query using
three independent approaches.  First, a \textbf{Python-UDF baseline}:
we implemented custom functions---built from \texttt{groupby},
...
semantics~\cite{Python_UDF}.  Second, a \textbf{Trino baseline}: we
ran the equivalent \texttt{MATCH\_RECOGNIZE} statement in Trino
(version~473) and treated its output as the reference result.  Third,
our \textbf{proposed engine}: we executed \texttt{MATCH\_RECOGNIZE}
...
\noindent Throughout our evaluation across multiple test scenarios, we
compared the results from all three approaches on a row-by-row basis,
confirming that our \texttt{MATCH\_RECOGNIZE} output exactly matched
that of Trino (see Table~\ref{tab:sql-match-recognize-examples}).  Both
Trino and our implementation produced identical rows and measure
values.  In contrast, the UDF method involved significantly more code
and was more prone to errors during development~\cite{Python_UDF}.
...
\end{minipage} \\
\hline
\textbf{Our Proposed \texttt{MATCH\_RECOGNIZE} Results:} \\
\begin{minipage}{0.90\textwidth}
\begin{verbatim}
  customer_id  start_date    end_date  bottom_price
...
Our methodology involved testing 25 test cases across 5 SQL patterns
and 5 input sizes (25\,K, 35\,K, 50\,K, 75\,K, 100\,K rows).  All
tests passed (100\% success).  The implementation demonstrated an
average throughput of 9\,838~rows/s, and the end-to-end runtime over
all 25 test cases was 157.44\,s, demonstrating pattern-invariant
performance and near-linear scaling with input size.
...
average-to-good products.  Useful for analysing quality improvement
...
\noindent
All patterns analyse product quality based on star ratings where
...
Table~\ref{tab:pattern_summary} summarises the performance
...
best throughput at 12\,455~rows/s.  The \texttt{complex\_nested}
pattern, while lower at 6\,630~rows/s, successfully handles intricate
nested structures.  The \texttt{alternation} pattern maintains good
throughput at 10\,774~rows/s, and \texttt{optional\_pattern} provides
strong performance at 11\,721~rows/s.
...
Table~\ref{tab:size_summary} presents performance metrics organised by
dataset size, averaging results across all five patterns.  The table
demonstrates remarkable throughput stability across scales, with less
than 8\% variation between the smallest (25\,K rows) and
largest (100\,K rows) datasets.
...
Table~\ref{tab:execution_times} presents a comprehensive matrix of
execution times for all pattern--size combinations.  The table
clearly illustrates the near-linear relationship between dataset size
and execution time, with execution times roughly doubling as dataset
size doubles.  The \texttt{complex\_nested} pattern consistently
...
6{,}436 to 12{,}799~rows/s (mean 9{,}711~rows/s).  Simple sequential
...
size increases, confirming linear scalability.

Table~\ref{tab:throughput} displays throughput measurements across
all test scenarios.  The table reveals remarkable throughput
consistency within each pattern across different dataset sizes
(standard deviation less than 5\% for most patterns).
...
measurements showed negative artefacts due to garbage collection
timing.
...
rows), demonstrating efficient memory management regardless of
dataset size.  Notably, the 100\,K-row tests consume less peak memory
(maximum 39.15\,MB) than the 75\,K-row tests, further validating the
non-linear memory behaviour.
...
1.36\,MB compared to \texttt{alternation}'s 30.12\,MB, demonstrating
...
implementation demonstrates efficient memory utilisation, with peak
...
usage.  The table demonstrates the implementation's efficient memory
management, with peak memory remaining under 43\,MB across all 25
test cases.  The maximum peak memory of 42.34\,MB occurs at 75\,K
...
Total Tests             & 25 \\
Success Rate            & 100\% \\
Total Execution Time    & 159.39\,s \\
Average Execution Time  & 6.38\,s \\
Average Throughput      & 9{,}711 rows/s \\
Min Throughput          & 6{,}436 rows/s \\
Max Throughput          & 12{,}799 rows/s \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:overall_stats} presents the comprehensive test
statistics aggregated across all 25 test cases.  The system achieves
a 100\% success rate across all patterns and dataset sizes, with no
...
from simple sequences to complex nested structures, and demonstrates
viability for production use cases in domains requiring sequential
pattern detection over e-commerce data.
