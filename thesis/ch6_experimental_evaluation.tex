...
\texttt{MATCH\_RECOGNIZE} query was then executed in Trino to detect three consecutive
...
and sorted by company and trading date.  For each company, the baseline computes a
...
All three approaches---the proposed engine, Trino~473, and the
...
\noindent Throughout the evaluation across multiple test scenarios, the results
from all three approaches were compared on a row-by-row basis,
confirming that the \texttt{MATCH\_RECOGNIZE} output exactly matched
that of Trino (see Table~\ref{tab:sql-match-recognize-examples}).  Both
Trino and the proposed implementation produced identical rows and measure
...
Pandas logic.  The next chapter summarises the thesis contributions,
