...
snowflake2025match_recognize,petkovic2022rpr}.  The supported context
differs: some allow row patterns only in the \texttt{FROM} clause,
while others also support a window.  The availability of row-pattern
functions such as \texttt{CLASSIFIER()} and \texttt{MATCH\_NUMBER()}
varies by system and may be restricted to specific clauses or output
modes.  Aggregate support varies too, including limits on
\texttt{DISTINCT} aggregates and aggregate arguments.  And features
such as empty matches, exclusions, reluctant quantifiers, and skip
...
\begin{enumerate}
  \item A \textbf{Pandas-native engine} for the evaluated SQL:2016
    ...
    skip modes, navigation functions, and aggregates.  Its pattern
    constructs include quantifiers, alternation, grouping, anchors,
    exclusions, and \texttt{PERMUTE}.  It runs in-process and needs no
    database server or cloud warehouse.  Internally it is organised as
    five core stages---parsing, semantic validation, logical-plan
    construction, pattern compilation, and execution
