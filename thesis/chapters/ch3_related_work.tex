...
Section~\ref{sec:bg-match-recognize} introduced the three optional
row-pattern features in the SQL standard: R010 (\texttt{FROM}-clause
patterns), R020 (window patterns), and R030 (full aggregate
support).
In practice, vendor documentation normally
describes the supported syntax instead of declaring conformance by
feature code.  The comparison therefore describes the documented scope
directly (See Table~\ref{tab:engine-comparison}).  It does not claim that
all systems
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l>{\raggedright\arraybackslash}Xll}
    ...
    Apache Flink & Streaming SQL subset over a complex-event-processing
      engine &
...
Oracle introduced SQL row-pattern recognition in
Database~12c~\cite{OracleMR}.  Oracle~21c, the version used in this
thesis, is a main reference for relational
\texttt{MATCH\_RECOGNIZE} semantics~\cite{oracle2021dwhsg,book}.
Its documentation places \texttt{MATCH\_RECOGNIZE} in a table
expression; it does not document the R020 window form.  It also
prohibits some nesting and correlated-subquery forms in
\texttt{MEASURES} and \texttt{DEFINE}.
...
sources~\cite{trino2022,trino2021row}.  Trino~473, the version used in
this thesis, is a semantic reference for selected results and a
performance baseline.  Its documentation covers
\texttt{MATCH\_RECOGNIZE} in the \texttt{FROM}
clause~\cite{trino2022} and also documents row pattern recognition in
window structures~\cite{TrinoDocumentation}.  This gives Trino broad
documented coverage among the reviewed open-source systems.  It is
still a query server rather than an in-process DataFrame library.
...
\texttt{MATCH\_RECOGNIZE} through Flink SQL, the SQL interface to
Flink's streaming engine, backed by its Complex
...
streaming use cases.  The current Flink documentation lists unsupported
features including pattern groups, subsequence alternation,
\texttt{PERMUTE}, anchors, exclusions, \texttt{ALL ROWS PER MATCH},
\texttt{SUBSET}, physical offsets (\texttt{PREV}/\texttt{NEXT}), and
\texttt{DISTINCT} aggregations~\cite{flink}.\par\endgroup  Flink fits continuous event streams, but it
...
work~\cite{duckdb2026roadmap}.  The database group at the University
of T\"ubingen instead translates row-pattern queries into SQL that
DuckDB already supports.  Their paper \emph{Democratize
\texttt{MATCH\_RECOGNIZE}!} presents this route, and a bachelor's
thesis and further student research projects in the same group develop
it~\cite{lambrecht2025democratize,findling2025bringing,TURP2,
TURP}.  A transpiler is a source-to-source
compiler: it rewrites code into another language at the same level of
abstraction, rather than down to machine
code~\cite{mogensen2024compiler}.  Here both sides are SQL.  The input
is a \texttt{MATCH\_RECOGNIZE} query, and the output is SQL that
DuckDB already runs.  Their transpiler produces non-recursive and
recursive common table expressions (CTEs) and then executes the
translated SQL in DuckDB.  A CTE is a named temporary result set,
written with \texttt{WITH} at the start of a query and referenced
later in the same statement.  A recursive CTE may also refer to
itself, which is how plain SQL can walk a row sequence step by step.
...
Optimization work on row patterns shares one broad goal: reduce
unnecessary work before or during the automaton scan without changing
the query result.  The approaches differ in where they place that
work.
...
then runs a non-deterministic finite automaton (NFA) matcher on the
smaller input.  Cost estimates for the joins and the automaton guide
this choice.
...
execution path when an optimization does not apply.
...
workloads that already live in those environments.  For a smaller
dataset that fits in the memory of one machine, it is unnecessary
cost: the system must be installed and configured, and the data must
be moved into it before the first pattern query can
run~\cite{8509400,Evaluation,in-memory_processing}.
...
\texttt{MATCH\_RECOGNIZE}-style primitive~\cite{pandas233api}.  The
logic must therefore be assembled by hand, as above, or sent to
another engine.
...
\paragraph{Research gap in the Pandas ecosystem.}
...
\begin{enumerate}
  \item A \textbf{Pandas-native engine} that executes an R010-style
    subset of SQL:2016 \texttt{MATCH\_RECOGNIZE} directly on a
    DataFrame.  It supports the main clauses, output modes, skip modes,
    navigation functions, and aggregates in the forms this thesis
    evaluates.  Its pattern constructs include quantifiers,
  ...
  \item An \textbf{optimized and guarded execution design} with
    compiled fast paths, vectorized predicate evaluation, compact
    output construction, bounded caching, and partition-scoped data
    handling.  One rule governs the layer: a faster path may never
    change the answer.  Each path declares an activation condition and
    runs only when that condition is proven to hold.  Row-local
    predicates become column masks, an already-ordered input skips the
    sort, eligible queries fuse matching with output construction, and
    compiled automata are reused across queries.  Separate budgets
    bound cache capacity, automaton construction, and exact search.
    Conditions outside a fast path use the general evaluator, and
    differential tests compare each eligible fast path with the
    general path on the same input and output mode.
    Chapter~\ref{chap:implementation} presents the compilation,
    execution, and optimization layers.

  \item \textbf{Theoretical and empirical evaluation} of the engine.
    The theoretical analysis gives time and space bounds for ordering,
    matching, output construction, and pattern complexity.  The
    empirical study covers correctness, SQL feature coverage,
    usability, execution time, throughput, memory, and scalability.
    It runs on synthetic reference cases, Amazon UK Products~2023 up
    to 2.22~million rows, and Amazon Reviews~2023 up to
    227.9~million rows~\cite{asaniczka2023amazon,hou2024amazonreviews},
    and compares the engine directly with Trino~473 and Oracle~21c EE.
