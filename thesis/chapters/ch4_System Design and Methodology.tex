...
(See Figure~\ref{fig:system-architecture}).  \emph{Parsing} creates an
...
\paragraph{Worked pipeline example.}
\label{subsec:meth-worked-pipeline}
The architecture is applied here to the \texttt{orders} example (See
Figure~\ref{fig:orders-pipeline}).  The figure carries both inputs, an SQL query
and an
eight-row DataFrame, and traces them through the five stages (See
...
structure and the two languages it mixes.  The parser must handle both
at once.  Listing~\ref{lst:mr-full-syntax} gives the form the core path
accepts.
...
This process is shown as a pipeline using \texttt{SELECT price FROM
orders} (See Figure~\ref{fig:visitor-dispatch}).  The visitor records
...
This structure is shown for the worked \texttt{orders} query (See
Figure~\ref{fig:meth-ast}).  The outer clauses and the row-pattern clause
...
The logical plan is resolved as an execution configuration.  It collects
the typed AST fields pulled from the \texttt{MatchRecognizeClause} that compilation and matching need
(See Table~\ref{tab:logical-plan}).  Later stages do not reparse the full
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llp{6.2cm}}
    ...
    \texttt{order\_by}       & \texttt{List[str]}
      & Column names or supported expressions used for
        intra-partition ordering \\[4pt]
    ...
    \texttt{measure\_semantics} & \texttt{Dict[str, str]}
      & Measure alias $\to$ \texttt{"RUNNING"}/\texttt{"FINAL"}, per the priority rules (See Figure~\ref{fig:measure-sem-resolution}) \\[4pt]
...
implicit true predicate.  Measure semantics follow a set of priority rules
(See Figure~\ref{fig:measure-sem-resolution}).  An explicit
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[msstart, below=of scan] (current)
      {\textbf{For each measure}\\[-2pt]
       Apply the first matching rule.};

    \node[mstest, below=of current] (explicit)
      {1.\ Does the measure have an explicit \texttt{RUNNING} or
       \texttt{FINAL} prefix?};
