...
resolve the configuration before compilation.  These settings remain
in the execution context while the pattern is compiled.  The compiler
converts the pattern into a non-deterministic finite automaton (NFA)
and then a deterministic finite automaton
(DFA)~\cite{georgiou2025streaming}.  A
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\scriptsize]
      at ([yshift=-0.91cm]excompile.center)
  ...
  \end{tikzpicture}
  \caption{Worked \texttt{orders} pipeline from
    Listing~\ref{lst:orders-mr}.  The query is processed in
    Stages~1--4; the DataFrame joins at Stage~5 and produces one match.}
  \label{fig:orders-pipeline}
\end{figure}

The input table accounts for all eight rows.  Stage~4 shows the four
reachable DFA states produced by the current compiler; the sets below
them are the corresponding NFA-state subsets.  The transition labels
use the predicates validated in Stage~2.  During execution,
...
\texttt{LAST} (\textsc{Running}).  If any measure has an explicit
prefix, an unprefixed measure that is still unresolved is
\textsc{Final}.  The earlier navigation rules keep their priority.
For example, an unprefixed \texttt{LAST} expression remains
\textsc{Running}.  If no measure has an explicit prefix, measures
beginning with \texttt{COUNT}, \texttt{SUM}, \texttt{ARRAY\_AGG},
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[msdeclared, right=of explicit] (stated) {declared mode};
  ...
  \end{tikzpicture}
  \caption{Priority rules for resolving \texttt{RUNNING} and
    \texttt{FINAL} measure semantics.}
