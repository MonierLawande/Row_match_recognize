...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[operation] (dispatch)
      ...
      {\textbf{b. Call operation}\\Node calls its\\matching operation.};
...
Two visitors build the engine's compact, typed AST.  One handles the
outer query.  The other handles the row-pattern clause.
Its root holds the outer \texttt{SELECT} and
\texttt{FROM} parts and the row-pattern clause.  Each sub-clause is
stored as a structured record with named fields.  Later stages can
work with these query parts instead of reading the raw SQL again.
