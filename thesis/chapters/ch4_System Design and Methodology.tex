...
class.  Choosing the right method takes two steps, because neither
side alone knows enough: the visitor knows the operation, and the node
knows its own type.  Each node is therefore handed the visitor and
calls back the method named for that node's type.  Two cooperating visitors handle the outer query and the
\texttt{MATCH\_RECOGNIZE} clause.  They produce a compact, typed AST.
Its root holds the outer \texttt{SELECT}/\texttt{FROM} and
