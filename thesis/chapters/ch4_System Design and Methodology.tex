...
outer query.  The other handles the row-pattern clause.  This matters
here because ANTLR4 generates the node classes, so they cannot be
edited to carry the work.  The AST root holds the outer
\texttt{SELECT} and \texttt{FROM} parts and the row-pattern clause.  Each sub-clause is
