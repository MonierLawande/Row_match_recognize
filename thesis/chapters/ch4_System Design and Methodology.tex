...
Section~\ref{sec:meth-logical} logical-plan construction.  Stages~4
and~5 belong to Chapter~\ref{chap:implementation}.  The figure shows
all five because the two chapters form one pipeline.
...
query AST compact.  Figure~\ref{fig:meth-ast} shows the tree for the
worked query.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    astfield/.style={anchor=west, font=\footnotesize, text=black!70},
    astleaf/.style={draw=orange!65!black, rounded corners=2.5pt,
                    line width=0.7pt, fill=orange!10, anchor=west,
                    inner xsep=4pt, inner ysep=2.6pt,
                    font=\footnotesize\ttfamily},
    spine/.style={draw=black!55, line width=0.6pt}
  ]
    \def\ra{-0.64} \def\cf{4.75} \def\cl{7.30}
    ...
    \node[astfield] at (\cf,1*\ra)  {\texttt{SelectItem}};
    ...
    \node[astleaf] at (\cl,1*\ra)  {"customer\_id" \dots\ "bottom\_price"};
    \node[astleaf] at (\cl,2*\ra)  {"orders"};
    \node[astleaf] at (\cl,4*\ra)  {"customer\_id"};
    \node[astleaf] at (\cl,5*\ra)  {"order\_date", ASC};
    \node[astleaf] at (\cl,6*\ra)  {"A.order\_date" AS start\_date};
    \node[astleaf] at (\cl,7*\ra)  {"C.order\_date" AS end\_date};
    \node[astleaf] at (\cl,8*\ra)  {"C.price" AS bottom\_price};
    \node[astleaf] at (\cl,9*\ra)  {"A B C"};
    \node[astleaf] at (\cl,10*\ra) {"B.price < A.price"};
    \node[astleaf] at (\cl,11*\ra) {"C.price < B.price"};
    ...
    \foreach \y in {1,2,4,5,6,7,8,9,10,11}
      \draw[spine, dotted] (\cf-0.12,\y*\ra) -- (\cl-0.06,\y*\ra);
    ...
    \node[anchor=west, font=\scriptsize, text=black!75]
      at (0,12*\ra) {Blue nodes are typed dataclasses.  Shaded leaves stay
                     as strings and are parsed later by the restricted
                     evaluator.};
  \end{tikzpicture}
  \caption{Typed AST for the worked \texttt{orders} query of
    Figure~\ref{fig:orders-pipeline}.}
  \label{fig:meth-ast}
\end{figure}
...
Trino's row-pattern tests.  General
