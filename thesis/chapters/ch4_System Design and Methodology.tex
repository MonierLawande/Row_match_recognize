...
\emph{Visitor} pattern~\cite{robson2022designpatterns}.  A visitor
keeps the walk over a tree separate from the classes of its nodes, so
a new operation can be added without editing them.  That matters here
because ANTLR4 generates both the node classes and the visitor base
class.  Two cooperating visitors handle the outer query and the
\texttt{MATCH\_RECOGNIZE} clause.  They produce a compact,
