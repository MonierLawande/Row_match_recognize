...
Pandas partitions.  Guarded fast paths reduce repeated Python
work without changing the supported semantics.
Section~\ref{sec:meth-automata} covers Stage~4: pattern compilation.
Section~\ref{sec:meth-pandas} covers Stage~5: DataFrame execution.
Section~\ref{sec:bg-performance} explains the optimization layer.
...
This section covers Stage~4 of the architecture (See
Figure~\ref{fig:system-architecture}).  Stage~3 resolves the execution
settings.  Stage~4 turns the pattern into the automata used for
matching.  These are separate tasks, completed before the rows are
scanned in Stage~5.
...
recognition task~\cite{HopcroftUllman}.

A finite automaton is a small machine with a fixed set of states.  It
starts in one state and reads the rows one at a time.  A row it accepts
moves it along a transition to another state.  If it arrives at an
accepting state, the rows it has read form a match.  Nothing else is
remembered: the current state carries everything the machine knows so
far.  That is what \emph{finite} means here.  The engine therefore
compiles the pattern before it scans the data.
...
preference order.  A simplified NFA for $A^{+}\,B?$ accepts the same row
sequences as the compiled pattern (See Figure~\ref{fig:meth-nfa}).
Intermediate wrapper states used during construction are not
shown.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[fragmenttext] at (6.88,2.43)
      {$B?$ \enspace optional: zero or one \texttt{B} row};
    ...
    \node[role] at (3.15,-1.30)
      {accept without \texttt{B}\\(\texttt{B} is optional)};
  ...
  \end{tikzpicture}
  \caption[Language-equivalent NFA for $A^{+}\,B?$]{Language-equivalent
    NFA for $A^{+}\,B?$.  The \texttt{B} row is optional: after one or
    more \texttt{A} rows, $q_1$ accepts without \texttt{B}, while $q_2$
    accepts after one \texttt{B} row.}
  \label{fig:meth-nfa}
\end{figure}
...
\texttt{\{$i$,$j$\}} into minimum and maximum counts together with a
...
\texttt{PERMUTE} over $m$ variables represents all $m!$ orders.
Construction is therefore guarded at eight variables and an estimated
50{,}000 branches.  The two numbers are one decision: eight is the
largest $m$ whose $m!$ still fits under the branch ceiling, since
$8! = 40{,}320$ stays below it while $9! = 362{,}880$ passes it seven
times over.  Both are checked before any fragment is built: the
variables are counted, and the branches are estimated from the pattern.
Exceeding either guard raises an explicit complexity error, so the
engine never starts a construction it cannot finish.  Rows inside \texttt{\{-\ \ldots\ -\}} still take
...
row, that is, the full set of states the NFA could be in.  Predicates or structural choices that depend on tentative
...
a structural ceiling of 50{,}000 states.  This ceiling counts DFA
states, and it is checked after the construction guards above, so a
pattern that passes those can still be stopped here.  The memory
budget may lower that ceiling on a constrained system.  Reaching any limit raises a
...
perform full DFA minimization, which would also merge states that
behave identically for every remaining input.  Transition metadata is preserved so
...
This section covers Stage~5.  It uses the execution settings from
Stage~3 and the compiled pattern from Stage~4
(Section~\ref{sec:meth-automata}).  Logical planning decides how the
query should run; pattern compilation prepares the automata; execution
applies them to the input rows.

Pandas~\cite{McKinney2010} stores each DataFrame column as an
array.  This suits both row-by-row matching and the \emph{vectorized}
fast paths in Section~\ref{sec:bg-performance}, which test a condition
on a whole column at once instead of one row at a time.  The system
boundary appears in the architecture overview (See
Figure~\ref{fig:system-architecture}); Stage~5 is expanded here together
with the preceding planning and compilation stages (See
Figure~\ref{fig:exec-pipeline}).
Stage~5 takes the execution settings with the compiled pattern and the
Pandas DataFrame.  Its five logical execution steps are labelled
5a--5e: partitioning, ordering, match
enumeration with predicate evaluation, measure evaluation, and result
materialization.  They are operations within Stage~5, not another set
of architecture stages.  An optimized physical path may fuse some
operations without changing this logical flow.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    % Show planning and compilation separately before Stage 5.
    \node[context, text width=4.8cm] (sql) at (6.00,7.00)
      {\textbf{SQL query}\\[-1pt]
       {\scriptsize\texttt{MATCH\_RECOGNIZE}}};
    \node[plan, text width=9.8cm] (logical) at (6.00,5.20)
      {\textbf{Stage 3 --- Logical-plan construction}\\[2pt]
       Resolve keys, conditions, measures, skip policy and output mode};
    \node[plan, text width=9.8cm] (compiled) at (6.00,3.25)
      {\textbf{Stage 4 --- Pattern compilation}\\[2pt]
       Pattern tokens $\longrightarrow$ NFA $\longrightarrow$ complete DFA};
    \draw[dataflow] (sql) --
      node[upstreamlabel, right, align=left]
      {Stages 1--2:\\parsing and validation} (logical);
    \draw[dataflow] (logical) --
      node[upstreamlabel, right] {Resolved pattern and settings} (compiled);

    % Runtime data lane.
    \draw[phasebar] (1.25,1.56) -- (10.75,1.56);
    \node[stageheader] (exec) at (6.00,1.56)
      {Stage 5 --- DataFrame execution};
    \draw[controlflow] (compiled.south) --
      node[upstreamlabel, right, align=left]
      {Execution settings\\and compiled pattern} (exec.north);

    \node[dataframe] (data) at (0,0)
      {\textbf{Input}\\[-1pt]
       {\scriptsize Pandas DataFrame}\\[3pt]
       {\scriptsize
       \begin{tabular}{@{}ccc@{}}
    ...
    \node[station] (s1) at (2.10,0) {5a};
    \node[station] (s2) at (4.00,0) {5b};
    \node[station] (s3) at (5.95,0) {5c};
    \node[station] (s4) at (7.95,0) {5d};
    \node[station] (s5) at (9.85,0) {5e};
  ...
  \end{tikzpicture}
  \caption[Planning, compilation, and DataFrame execution]{Logical
    planning (Stage~3), pattern compilation (Stage~4), and DataFrame
    execution (Stage~5).  Steps 5a--5e are operations within Stage~5.}
...
computation (See Table~\ref{tab:skip-modes}).  Empty matches always
advance
...
row.  The row counts and measure contexts are summarized together (See
...
Their activation boundaries are listed together, and each technique below
carries the name used there (See Table~\ref{tab:perf-summary}).

\begin{table}[htbp]
  \centering
  \caption[Performance optimizations, purpose and activation]{Performance
    optimizations: purpose, activation condition, and benefit.  The
    technique names match the bold paragraph headings that follow.}
  \label{tab:perf-summary}
  \small
  \setlength{\tabcolsep}{3pt}
  % narrow columns: ragged right and no hyphenation, so short labels
  % are not broken across lines ("Avoid copy-ing")
  \begin{tabularx}{\textwidth}{%
    >{\raggedright\arraybackslash\hyphenpenalty=10000\exhyphenpenalty=10000}p{2.75cm}%
    >{\raggedright\arraybackslash\hyphenpenalty=10000}p{2.05cm}%
    >{\raggedright\arraybackslash}X%
    >{\raggedright\arraybackslash}X}
    \toprule
    \textbf{Technique} & \textbf{Purpose}
      & \textbf{Activation condition} & \textbf{Primary benefit} \\
    \midrule
    Ordered-input check
      & Avoid copies
      & Requested \texttt{ORDER BY} sequence already present
      & Avoids an unnecessary sort and DataFrame copy \\ \addlinespace[5pt]
    Vectorized condition masks
      ...
      & Replaces repeated Python evaluation with column operations
        and exact prefilters \\ \addlinespace[5pt]
    ...
    Fused output drivers
      ...
      & Avoids intermediate match and row dictionaries \\ \addlinespace[5pt]
...
\paragraph{Ordered-input check.}
Sorting is the one cost that can be removed outright.  Before sorting a
partition, the engine tests whether it already follows the requested
\texttt{ORDER BY} sequence.  If it does, the sort and the DataFrame
copy it would produce are both skipped.  Section~\ref{subsec:eval-ordering}
measures the engine with and without that shortcut.

\paragraph{Vectorized condition masks.}
...
\paragraph{Linear run-length plan.}
...
\paragraph{Fused output drivers.}
...
\paragraph{Compiled measure and prefix plans.}
...
\paragraph{Lazy rows and output projection.}
...
materialized rows or whole-column working arrays.

\paragraph{Budgets.}
The current implementation uses separate resource limits, called
budgets.  The compiled-pattern cache limits the number and estimated
size of entries kept for reuse.  It removes cached entries when space
is needed and leaves an entry uncached if it is too large to fit.
DFA construction limits both the number of states built and the
construction iterations.  Exact search limits the exploration steps
from each candidate start row.  If construction or search cannot finish
within its budget, the engine raises an explicit error rather than
returning an incomplete result.  These checks do not cap total process
memory, which also includes the input, temporary data, and output.
