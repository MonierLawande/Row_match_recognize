...
the length of the expression, by recursing over the expression
structure: a single symbol becomes a two-state fragment; a
concatenation merges the accept state of the left fragment with the
start state of the right; an alternation adds a fresh start state
fanning out to both branches via $\varepsilon$-edges and a fresh
accept state collecting them; a Kleene star adds a skip
$\varepsilon$-edge (zero repetitions) and a loop-back
$\varepsilon$-edge (repetition); and a bounded quantifier
$r^{\{n,m\}}$ is expanded into $n$ mandatory copies followed by
$m{-}n$ optional copies~\cite{DragonBook}.  The implementation uses
...
\begin{tabular}{lll}
  ...
  \texttt{\{$n$,\}} & $\to$ & $(n,\;\infty,\;\mathit{True})$ \\
  \texttt{\{,$m$\}} & $\to$ & $(0,\;m,\;\mathit{True})$ \\
  \texttt{\{,\}} & $\to$ & $(0,\;\infty,\;\mathit{True})$ \\
...
\begin{itemize}
  ...
  \item \textbf{Reluctant} (\texttt{greedy = False}): the parser
    records reluctant quantifiers (a trailing \texttt{?}, including
    the bounded forms \texttt{\{$n$,$m$\}?} and \texttt{\{$n$,\}?})
    through the token's \texttt{greedy} flag.  The pattern analysis
    and the matcher's search order honor the flag, preferring the
    shortest admissible repetition count before longer ones; this
    behavior is verified against Trino's 32-case quantifier
    reference matrix.
\end{itemize}
...
each associated with a Boolean \texttt{DEFINE} predicate.  The
tokenizer produces the flat list
\texttt{[LITERAL('A'), LITERAL('B','+'), LITERAL('C','?')]}.
The builder creates the global start/accept pair, then for each
token a two-state condition fragment
$q_s \xrightarrow{C(v)} q_e$; the \texttt{+} on \texttt{B} adds a
greedy loop-back $\varepsilon$-edge plus an exit edge inside an
envelope pair, and the \texttt{?} on \texttt{C} adds a bypass
$\varepsilon$-edge that skips the fragment.  Sequential fragments
are chained with $\varepsilon$-transitions and the tail is connected
to the accept state, giving twelve NFA states in the current
implementation.  The automaton accepts exactly
$A \cdot B^{+} \cdot C^{?}$---the loop edge implements
one-or-more repetition and the bypass edge implements optionality---
and is then passed to \texttt{DFABuilder.build()} for subset
...
Figure~\ref{fig:exec-pipeline} distinguishes these six execution
steps from the five top-level phases of the engine.  The first four
phases transform the SQL query into a compiled matcher; the shaded
fifth phase combines that matcher with the input DataFrame.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    % The SQL path forms one vertical, ordered chain of engine phases.
    \node[phase] (parse)
  ...
  \end{tikzpicture}
  \caption{Five-phase engine pipeline.  Phases~1--4 transform the SQL
    query into a compiled DFA; the shaded execution phase applies it to
    the input DataFrame through six ordered steps and returns a result
    DataFrame.}
...
\texttt{DEFINE} predicates may also contain aggregate functions and
\texttt{CLASSIFIER()}, which SQL:2016 evaluates with \emph{running}
semantics over the tentative match: the aggregate ranges over the
rows matched so far \emph{plus} the current row under the label
being tested.  The evaluator
(\texttt{\_handle\_define\_aggregate()} in
\texttt{src/matcher/condition\_evaluator.py}) implements this for
\texttt{SUM}, \texttt{AVG}, \texttt{MIN}, \texttt{MAX},
\texttt{COUNT}, \texttt{ARBITRARY}, \texttt{ARRAY\_AGG}, and the
two-argument \texttt{MAX\_BY}/\texttt{MIN\_BY}.  A label-qualified
argument such as \texttt{sum(B.price)} restricts the aggregate to
the rows assigned to that variable (resolving \texttt{SUBSET}
aliases to their member unions), while an unqualified argument
ranges over all rows of the tentative match; argument expressions
are re-evaluated per contributing row, so forms such as
\texttt{max(value + MATCH\_NUMBER())} or
\texttt{min(lower(CLASSIFIER()))} are supported.  Conditions
containing such state-dependent constructs are flagged by
\texttt{compile\_condition()} and bypass the condition cache, since
their value depends on the evolving match state rather than on the
row alone.
...
The greedy DFA walk assumes that a predicate's outcome depends only
on the row under test and previously fixed bindings.  This
assumption breaks for \emph{state-dependent} conditions---aggregates
or \texttt{CLASSIFIER()} in \texttt{DEFINE}---where accepting a row
under one label changes the value of predicates tested later, and
for patterns whose preference order can only be established by
trying alternatives (a quantified group containing an alternation
with a multi-element branch).  For these queries the matcher
dispatches to a dedicated searcher,
\texttt{\_find\_single\_match\_condition\_backtracking()}, which
performs a preference-ordered depth-first search directly over the
parsed pattern structure: at each position it tentatively assigns
the row to the highest-preference viable alternative, re-evaluates
the affected conditions under that tentative assignment, and
backtracks when a later condition fails.  A step budget bounds the
worst-case exponential search; queries that stay within the budget
receive exact SQL:2016 preference semantics identical to Trino's
tentative-label matching (verified by the reference test suite,
including the set-partitioning and balancing-sums queries that
cannot be matched greedily).  A gate function inspects the compiled
query and routes only the affected queries to this searcher, so the
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{9cm}}
    ...
    \textsc{To First} $V$ &
      ...
      ($V$ may also be a \texttt{SUBSET} alias, denoting the union
      of its members).  Requires $V$ to be declared; enforced at
      compile time.  Skipping to the first row of the match itself
      is reported as a runtime error, since it would loop forever. \\[4pt]
    \textsc{To Last} $V$ &
      $i \leftarrow \max(\mathit{bindings}[V])$. The scan
      resumes at the \emph{last} row assigned to variable $V$ (or
      \texttt{SUBSET} alias).  May overlap if $V$'s last row
      precedes the match end.  If $V$ matched no row in the match,
...
The optimizations fall into two groups.  The first group consists of
\emph{compiled execution fast paths} inside the matcher itself
(\texttt{src/matcher/matcher.py}): specialized code paths that are
selected per query after inspecting the pattern and the
\texttt{DEFINE} predicates, and that fall back to the general
evaluator whenever a construct they do not cover appears.  Every
fast path is required to be output-identical to the general path;
this is enforced with differential tests that compare complete
result DataFrames between the two paths.  The second group consists
of \emph{auxiliary infrastructure}---caching, memory management, and
scheduling---described afterwards.

% ................................
\subsection{Vectorized Navigation Conditions}
\label{subsec:opt-vectorized-nav}
% ................................

\texttt{DEFINE} predicates built from comparisons of the current
row's columns and \emph{physical} navigation
(\texttt{PREV($c$,$k$)}/\texttt{NEXT($c$,$k$)}, which address row
$i \mp k$ of the partition regardless of variable assignment) do not
depend on the match state at all.  For such predicates,
\texttt{\_vectorized\_eval\_row\_local\_ast()} evaluates the whole
condition once per partition as a column expression: navigation
becomes a Pandas \texttt{shift($k$)}, comparisons become vectorized
Boolean arrays, and the DFA simulation then reads precomputed
per-row truth values instead of calling a Python closure per row.
Predicates containing \texttt{FIRST}/\texttt{LAST},
\texttt{CLASSIFIER}, aggregates, cross-variable references such as
\texttt{PREV(A.x)} inside \texttt{DEFINE B}, or negation over a
shifted value (which requires SQL three-valued logic) retain the
scalar evaluator.

% ................................
\subsection{Row-Local DFA Decision Tables}
\label{subsec:opt-dfa-tables}
% ................................

When every \texttt{DEFINE} predicate of a pattern is vectorized as
above, the winning transition of each DFA state is itself a pure
function of the row index.  \texttt{\_build\_row\_local\_transition\_index()}
precomputes, for each DFA state, a byte array over the partition
whose $i$-th entry encodes the highest-priority transition whose
condition holds at row $i$ (255 encodes ``no transition'').  The
simulation loop then reduces to indexing \texttt{bytes} objects at C
speed.  Variable assignments are recorded as
$(\mathit{var}, \mathit{start}, \mathit{stop})$ \emph{run segments}
rather than per-row list appends, and are expanded into row lists
only for the accepted match, so failed exploratory walks never
materialize per-row structures.

% ................................
\subsection{Linear-Pattern Fast Path}
\label{subsec:opt-linear}
% ................................

Patterns that are a plain concatenation of quantified variables
(e.g.\ \texttt{A\{1,5\} B* C+}) admit a still simpler strategy.
\texttt{\_get\_linear\_plan\_run\_lengths()} precomputes, for every
variable, vectorized run-length tables (for each row, how many
consecutive rows starting there satisfy the variable's predicate),
and \texttt{\_linear\_search\_iterative()} searches the quantifier
bounds iteratively over these tables with a memo of failed
(token, position) states.  Because a suffix that fails from a given
token and row fails regardless of the match's start position, the
memo is shared across all start positions of a partition scan,
turning repeated backtracking into table lookups.

% ................................
\subsection{Fused ONE ROW PER MATCH Driver}
\label{subsec:opt-fused-driver}
% ................................

For queries using \texttt{ONE ROW PER MATCH} with
\texttt{AFTER MATCH SKIP PAST LAST ROW}, no anchors, and patterns
fully covered by the compiled matchers above,
\texttt{\_run\_fast\_one\_row\_pass()} fuses matching and output
production: matches are enumerated directly by the linear or DFA
fast path, and each output row is emitted straight from the match's
run segments through measure closures compiled once per query
(\texttt{\_compile\_segment\_measure\_closure()}).  The general
path's intermediate structures---per-match dictionaries, per-variable
index lists, and the \texttt{RowContext}---are never constructed.
This driver preserves element order inside aggregates, so its output
is identical to the general path's.

% ................................
\subsection{Compiled Measure Plans and Prefix Aggregation}
\label{subsec:opt-measure-plans}
% ................................

Measure evaluation follows the same compile-once philosophy.
\texttt{\_compile\_measure\_plan\_closure()} resolves column arrays,
null masks, and case-insensitive variable keys once per partition
pass instead of once per match.  For \texttt{ALL ROWS PER MATCH},
per-measure semantics analysis is hoisted out of the per-row loop,
with dedicated paths for \texttt{CLASSIFIER()} and
\texttt{MATCH\_NUMBER()}.  Arithmetic over \emph{running} aggregates
(e.g.\ \texttt{RUNNING sum(A.value*A.score)/sum(A.score)}), which a
naive per-row evaluation makes quadratic, is compiled by
\texttt{\_compile\_running\_aggregate\_arithmetic\_plan()} into a
single $O(n)$ prefix pass over the match rows.
...
The chapter also described the optimization layer.  Its core is a
set of compiled execution fast paths selected per query: vectorized
evaluation of row-local \texttt{DEFINE} conditions, byte-encoded
row-local DFA decision tables with run-segment assignment recording,
a linear-pattern search with a shared failure memo, a fused
\texttt{ONE ROW PER MATCH} driver that emits output rows directly
from match segments, and compiled measure plans including an $O(n)$
prefix pass for arithmetic over running aggregates.  State-dependent
\texttt{DEFINE} conditions (aggregates and \texttt{CLASSIFIER}) are
routed to an exact preference-ordered backtracking searcher instead.
Auxiliary infrastructure---LRU-style pattern caching, parallel-aware
partition scheduling, partition-oriented memory management, and
condition caching/pre-warming---completes the layer.  Every fast
path is required to produce output identical to the general
evaluator, so these optimizations are orthogonal to the core
matching semantics and can be evaluated independently in the
