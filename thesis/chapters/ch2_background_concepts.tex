...
(See Figure~\ref{fig:intro-shapes}), as well as repeated or changing
behavior.
...
fit the problem.  The combination gives the best of both.  The syntax
is a readable way to write the pattern, and the automaton is an
efficient way to run it, one row at a time.
Later chapters use this connection to compile row
...
Possible applications that benefit from this thesis work span several
domains~\cite{OracleMR}.  The following examples show where the need
arises.

\paragraph{Financial Market Analysis.}
Detecting a V-shaped price pattern, a decline followed by a recovery,
requires an ordered sequence of price movements.  Successive
...
natural use of \texttt{MATCH\_RECOGNIZE}~\cite{OracleMR}.\par\endgroup
...
Listing~\ref{lst:mr-syntax}.  The standard requires \texttt{PATTERN}
and \texttt{DEFINE}.  The square brackets in the listing mark the
optional specifications.  Together, they control partitioning, order,
output, row classification, and overlap between matches.  The clause
thus mixes two languages: ordinary SQL for partitioning, ordering, and
expressions, and a regular-expression-like language for the pattern.
...
Splits the input into independent
...
and its own \texttt{NULLS FIRST} or \texttt{NULLS LAST} placement.
The evaluated engine implements these forms.


\item \textbf{MEASURES.}
  Defines computed expressions that extract values from
...
\item \textbf{ROWS PER MATCH.}
  \begin{itemize}
    \item \texttt{ONE ROW PER MATCH}: one summary row per complete
      match.  This is the default when the option is omitted.
    \item \texttt{ALL ROWS PER MATCH}: one output row for each included
      row in a match.  It takes one optional modifier:
      \texttt{SHOW EMPTY MATCHES} adds an output row when a pattern
      completes without consuming an input row,
      \texttt{OMIT EMPTY MATCHES} suppresses such rows, and
      \texttt{WITH UNMATCHED ROWS} also returns rows that no match
      covers.
  \end{itemize}

\item \textbf{AFTER MATCH SKIP.}
  Controls where the matcher resumes after finding a match:
  ...
  \begin{itemize}
    ...
    \item \texttt{SKIP PAST LAST ROW} -- resume from the row after
      match end (non-overlapping matches).  This is the default.
    ...
    \item \texttt{SKIP TO LAST} \textit{var} -- resume from the last
      row classified as \textit{var}.  Written without
      \texttt{FIRST} or \texttt{LAST}, \texttt{SKIP TO} \textit{var}
...
\item \textbf{PATTERN.}
  ...
  \begin{itemize}
    ...
    \item Quantifiers: \texttt{A*} (zero or more), \texttt{A+}
      (one or more), \texttt{A?} (zero or one), \texttt{A\{i\}}
      (exactly $i$), \texttt{A\{i,j\}} (between $i$ and $j$).
      A quantifier is \emph{greedy} by default: it takes as many rows
      as it can while the rest of the pattern still matches.  A
      trailing question mark makes it \emph{reluctant}, so it takes as
      few as possible: \texttt{A*?}, \texttt{A+?}, \texttt{A??}, or
      \texttt{A\{i,j\}?}.
...
\item \textbf{SUBSET.}
Defines a union variable from variables already used in
\texttt{PATTERN}.  It does not change the match.
...
\texttt{DEFINE}, and supported skip targets.  Of the eight
specifications, it is the one used least often in practice.


\item \textbf{DEFINE.}
Assigns Boolean predicates to pattern
...
\texttt{DEFINE} predicate uses the implicit true predicate.  The
standard still requires the clause itself.  The evaluated engine
relaxes that point and also accepts a query with no \texttt{DEFINE},
in which case every pattern variable is true.
...
\texttt{order\_date}.  A match starts with any row, because
\texttt{START} has no predicate in \texttt{DEFINE}.  It then has one or
