...
\texttt{MATCH\_RECOGNIZE} clause as \emph{supported},
\emph{partially supported}, or \emph{unsupported} against the
reference tests (Table~\ref{tab:feature_coverage}).  The clauses are
...
cold-start and just-in-time (JIT) compilation effects, followed by twenty
measured runs, each preceded
by a five-second idle window that sets the pre-query memory baseline.
Outliers are removed per metric with the $1.5\times$ interquartile range
(IQR) rule; cells
...
control groups.  A control group is the Linux kernel facility that caps
and reports the CPU and memory of a set of processes, and it is the
same instrument used for the engine.
...
used throughout the benchmark, with the detection goal of each.
...
sequentially.  Figure~\ref{fig:ordering_scenarios} shows the result.
...
(Oracle), and 3.8\% (Trino).  This supports the
