...
price the pattern tests (See Figure~\ref{fig:eval-sample-tables}).  The SQL
engines are the
...
on the main-matrix averages the gap is $0.12$\% or less.  For the main
benchmark, the two
...
(See Figure~\ref{fig:eval-sample-tables}).  The
...
ordering, and pattern detection.  The eight records used are listed in
full (See Table~\ref{tab:orders}).
...
The data are listed in full (See Table~\ref{tab:stock_price}).
...
(Dataset~1; See Table~\ref{tab:orders}).  The same detection is expressed
...
dialects~\cite{monierashraf2025oracle}.
...
For the \emph{correctness} criterion, all four implementations return the
same single match (See Table~\ref{tab:sql-match-recognize-examples}).  The
...
\par\smallskip{\small Source: \cite{monierashraf2025rowmatch}}
\end{table}
...
On the \texttt{stock\_price} data (See Table~\ref{tab:stock_price}) this
...
All four implementations return the identical three matches (See
Table~\ref{tab:stock-results}).
...
(See Table~\ref{tab:stock-results}), covering partitioning, ordering,
...
\par\smallskip{\small Source: \cite{petkovic2022rpr,monierashraf2025oracle}}
\end{table}
...
matches
(See Table~\ref{tab:vshape-results}).
...
For the \emph{correctness} criterion, all four implementations return the
same three matches (See Table~\ref{tab:vshape-results}).
...
The detection logic is counted on both sides in non-blank, non-comment
lines of code (LOC): the \texttt{MATCH\_RECOGNIZE} statement against the
Pandas detection function (See Table~\ref{tab:loc_comparison}).  Data
setup, the
imports, and the printing are shared, so they are left out of both.  The
declarative form takes 14 vs 23, 14 vs 25, and 16 vs 31 lines, and the
...
Dip (\texttt{orders}) & Hand-written Pandas function & loop / index / skip bookkeeping & 23 \\
\midrule
Stock (\texttt{stock\_price}) & \texttt{MATCH\_RECOGNIZE} statement & single declarative query & 14 \\
Stock (\texttt{stock\_price}) & Hand-written Pandas function & \texttt{groupby}/\texttt{shift}/index bookkeeping & 25 \\
\midrule
V-shape (\texttt{stock\_price}) & \texttt{MATCH\_RECOGNIZE} statement & quantified declarative query & 16 \\
V-shape (\texttt{stock\_price}) & Hand-written Pandas function & nested falling/rising run scan & 31 \\
\bottomrule
\end{tabularx}
}
\end{table}
...
(See Table~\ref{tab:feature_coverage}) and detected pattern types
(See Table~\ref{tab:pattern_type_coverage}), both checked against the
...
Quantifiers \texttt{* + ? \{i,j\}} & supported \\
...
Conditional and subquery ordering keys, general relational planning,
...
At the pattern level, the engine detects ten row-pattern types, each
listed with a regex example and a simple use case (See
Table~\ref{tab:pattern_type_coverage}).  A type counts as covered when at
least one test
...
dataset.  The two views are set side by side (See
Table~\ref{tab:complexity_analysis}).
...
precisely it is $\sum_h O(n_h \log n_h)$ over partitions of $n_h$ rows, which
...
The two cases are drawn side by side (See Figure~\ref{fig:complexity_bigO}).
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[box, right=of unordered] (sort)
      {Order check + sort\\$\scriptstyle O(n+\sum_h n_h\log n_h)$};
...
and~\ref{subsec:eval-memory} throughput and memory,
Section~\ref{subsec:eval-key-findings} the summary, and
Sections~\ref{subsec:eval-ordering}--\ref{subsec:eval-findings} sensitivity,
scalability, validity, and the key findings.
...
The five pattern families used throughout the benchmark are listed with
the detection goal of each (See Table~\ref{tab:pattern_definitions}).
They are the first five pattern types of the coverage table (See
...
The proposed-engine values describe the fixed benchmark revision recorded
in the appendix (See Appendix~\ref{app:provenance}).  The Oracle and Trino
values
...
rows (See Table~\ref{tab:match_counts}).
...
% a long code word near the end has no break point
\begingroup\emergencystretch=2em
The three systems run the same query bodies; they differ only in the
source table.  The benchmark protocol and the evidence used to reproduce
the comparison are recorded in the appendix (See
Appendix~\ref{app:provenance}).  Match counts per cell are identical
across the three systems, all of which use \texttt{ONE ROW PER MATCH} (See
Table~\ref{tab:match_counts}).  \texttt{optional\_pattern} yields the
...
despite fewer matches than \texttt{simple\_sequence}.\par\endgroup
...
Each family is averaged over the six sizes (See
Figure~\ref{fig:summary_by_pattern}): throughput on the left (higher is
better), time on the right
...
5.28\,s against the engine's 0.34\,s, a factor of $15.4$.  The values are
listed in the appendix (See Table~\ref{tab:pattern_summary}).
...
The same data are grouped by size (See Figure~\ref{fig:summary_by_size}).  The
engine is fastest at every size, holding 5.0--6.9\,M rows/s as input
grows.  Per size it is about $9.3$--$13.3\times$ faster than
Trino and $4.3$--$7.0\times$ faster than Oracle.  The values are listed in
the appendix (See Table~\ref{tab:size_summary}).
...
Each pattern has its own time-versus-size panel (See
Figure~\ref{fig:execution_times}).  The engine (blue) is the lowest curve
everywhere.  The margin widens most on
\texttt{complex\_nested}, where the databases bend upward past 400\,K
rows while the engine stays near-linear
(See Table~\ref{tab:execution_times}).
...
Throughput is plotted with one panel per pattern; higher is better (See
Figure~\ref{fig:throughput}).  Throughput is computed for each measured
run, as
...
\emph{Query memory} (See Figure~\ref{fig:query_memory};
Table~\ref{tab:memory_cache} in Appendix~\ref{app:benchmark-tables})
reports the incremental peak.  \emph{Footprint memory}
(See Figure~\ref{fig:footprint_memory}; Table~\ref{tab:memory_footprint})
...
per-query comparisons use the incremental peak instead (See
...
The overall statistics have four panels, each with one bar per system (See
Figure~\ref{fig:overall_stats}).  \emph{Average execution time} is the
mean of the 30 cell
...
that gives the speed, so the higher figure is expected rather than a
fault, and even its peak stays far below the 32\,GB limit.  Trino's
20.8\,MB is growth above an already-resident heap, not a like-for-like
figure.
...
for the highest per-query memory (See Table~\ref{tab:overall_stats}).
...
Averaged over the five patterns (See Figure~\ref{fig:avg_memory_by_size}),
the engine's per-query memory rises with size.  It stays the highest of
the three (See Table~\ref{tab:avg_memory_by_size}).
...
The footprint picture inverts (See Figure~\ref{fig:avg_footprint_by_size}).
The engine's single process stays well below the Trino and Oracle
containers at every size (See Table~\ref{tab:avg_footprint_by_size}).
...
detailed values are reported once, in the appendix (See
...
The two databases are set against the engine, which is drawn as a
$1\times$ reference (See Figure~\ref{fig:relative_comparison}).
...
mean incremental peak).  It is shown per cell (See
Figure~\ref{fig:query_memory}), and signed percentages are listed in the
appendix (See Table~\ref{tab:relative_comparison}).
...
\begingroup\emergencystretch=3em
The proposed-engine values describe the benchmark revision recorded in the
appendix (See Appendix~\ref{app:provenance}).  The Oracle and Trino values
were retained from the earlier
unordered campaign under the same one-core, 32\,GB, five-warmup,
twenty-run protocol.  As in the main benchmark, the systems ran
sequentially.  The result is shown by system (See
Figure~\ref{fig:ordering_scenarios}).\par\endgroup
...
(See Tables~\ref{tab:ordering_scenarios}--\ref{tab:ordering_by_size}).
...
The engine is compared on ordered (no sort) and shuffled (forced sort)
input (See Figure~\ref{fig:sort_cost_scaling}).  Both
...
order of magnitude
(See Table~\ref{tab:sort_cost_by_size}).  Over this measured range, the
...
the main benchmark.  The three systems are plotted for this test (See
...
of Trino (See Table~\ref{tab:worstcase_coverage}).
...
measured runs.  The proposed-engine values describe the benchmark revision
recorded in the appendix (See Appendix~\ref{app:provenance}).  The Oracle
and Trino measurements were
...
The results are plotted in two figures (See
Figures~\ref{fig:stress_volume} and~\ref{fig:stress_patterns}); full
values are in the appendix (See
...
Left is execution time and right is absolute peak memory (See
Figure~\ref{fig:stress_volume}).  Each size
...
five-family values, \texttt{complex\_nested} included, stay in the
appendix (See Tables~\ref{tab:stress_volume}--\ref{tab:stress_mem}).
...
(See Tables~\ref{tab:stress_volume}--\ref{tab:stress_mem}).
...
Throughput is again averaged over the same four common families (See
Figure~\ref{fig:stress_throughput}).  The engine holds
...
close to linear for all three (See Figure~\ref{fig:stress_time_grid}), and every
completed cell returned identical match counts
(See Tables~\ref{tab:stress_thr}, \ref{tab:stress_matches}).
...
three heavy families (\texttt{simple\_sequence}, \texttt{quantified}, and
\texttt{optional\_pattern}); for \texttt{quantified}, the times are 372\,s
...
the first to reach the time wall (See Table~\ref{tab:stress_volume}).
...
The query-memory counters give a diagnostic view, not a same-instrument
comparison (See Figure~\ref{fig:stress_memory}).  The figure places the engine's
cgroup incremental
...
(See Figure~\ref{fig:stress_volume} and
...
(See Table~\ref{tab:cross_system_correctness}).  The 30/30 agreement is a
