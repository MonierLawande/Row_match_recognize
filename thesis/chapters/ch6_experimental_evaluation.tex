...
30 cells per system and 90 in total.  The SQL engines are the
...
The warmups exist because a first run measures start-up rather than
steady state, and each system has something different to warm.  Trino
runs on the JVM, which compiles its own bytecode to machine code while
the early queries execute; that \emph{just-in-time} (JIT) step makes
them slower than later runs.  Oracle fills its shared pool and buffer
cache.  The engine imports its modules and compiles the pattern into
automata once, after which the pattern cache of
Section~\ref{sec:bg-performance} serves the repeats.

Single runs still vary after warmup, because the measurement shares one
machine with the operating system and its background work.  The widest
case in the main matrix was a Trino cell whose slowest run took
$2.09$\,s against $0.29$\,s for its fastest, a factor of $7.3$.
Outliers are therefore removed per metric with the $1.5\times$
interquartile range (IQR) rule: the IQR is the spread between the 25th
and 75th percentiles, and a run lying more than $1.5\times$ that spread
outside them is dropped.  The rule discarded 99 of the 1{,}800 measured
time runs, or $5.5$\%, and the reported mean for that Trino cell is
$0.33$\,s.  Cells report the mean of the retained runs with its
standard deviation.
Throughput is computed per run and then averaged the same way as time.
Each run contributes one rate, the dataset size divided by that run's
own time, and the cell reports the mean of those rates.  The obvious
alternative is to divide the dataset size by the cell's mean time, the
rate of the mean.  The two are not equal.  For one set of runs the mean
of the rates is always the larger, because a rate is one over a time,
and averaging that inverse gives a slow run less weight than averaging
the times does.  The relative gap grows roughly with the square of the
coefficient of variation, so it stays small while the runs are stable:
over the main matrix it is $0.11$\% for Oracle and $0.12$\% for Trino,
and its widest single cell is $1.18$\% for a Trino cell whose times
varied by $11.8$\%.  The mean of the rates is reported because it treats
every run as one observation, exactly as the time metric does.  Since
outliers are filtered per metric, the runs retained behind the two
numbers need not be the same ones.  For the main benchmark, the two
...
Four clauses do the work.  \texttt{PARTITION BY Company} splits the
table into one group per company, so a match never mixes rows of
\texttt{A} with rows of \texttt{B}.  \texttt{ORDER BY Price\_date}
puts each group in date order, which is what makes ``the row before''
mean anything.  \texttt{PATTERN (A B C)} asks for exactly three
consecutive rows.  \texttt{DEFINE} then attaches a condition to two of
those three names: \texttt{B} must be priced above the row before it,
and \texttt{C} above the row before it in turn.  \texttt{PREV} is what
reads that earlier row.  It steps back one row in the ordered
partition, not one row of the same variable.

\texttt{A} is left out of \texttt{DEFINE} on purpose.  A pattern
variable with no condition is implicitly true, so any row may open a
match and the rise is tested only from the second row onward.
...
\texttt{MATCH\_NUMBER()} restarts at~1 in each partition, so
company~\texttt{B}'s only match is numbered~1 rather than~3.
