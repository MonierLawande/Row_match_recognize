...
steady state.  Trino runs on the JVM, which compiles its own bytecode
...
can express; they are not the benchmark's five patterns, and neither is
the 30 of Section~\ref{subsec:eval-cross-system}, which is those five
run at six sizes.  The table can also be read in reverse, from a use
