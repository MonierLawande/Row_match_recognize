...
\begin{enumerate}
  ...
  \item \textbf{Support and verify useful row-pattern semantics.}  The
    ...
    clauses, output and skip modes, and navigation functions.  Its
...
Section~\ref{sec:related-contribution} develops these three and positions
them against the related systems; the engine itself is built as five core
stages (Chapter~\ref{chap:methodology}).

\paragraph{Evidence at a glance.}
The regression suite contains 546 automated test cases across 33 test
files, all passing~\cite{monierashraf2025rowmatch}.  They cover pattern families,
...
engine remained close to linear and kept a $3$--$5\times$ time advantage
over Oracle~EE and $6$--$8\times$ over Trino.
