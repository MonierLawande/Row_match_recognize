...
addresses, and Section~\ref{sec:intro-objectives} turns it into objectives,
contributions, and the evidence for them.  The remaining sections give the
structure of the thesis and the publications produced during it.
...
First, the rows are divided into independent groups (one customer, one sensor,
one bank card), and each group is put in order, usually by time.
...
The idea is shown on six days of one customer's prices (See
Table~\ref{tab:intro-rowlabels}).
...
Each name in the line is one of the row labels (See
Table~\ref{tab:intro-rowlabels}), and the order of the names is the order
the rows must appear in.  The symbol
...
length, which is what makes it a pattern.  This is the V shape (See
...
measured value is plotted against time.  Three names recur, shown side by
side (See Figure~\ref{fig:intro-shapes}).
...
data it queries.  This is the \texttt{MATCH\_RECOGNIZE} clause, and it is the
...
\texttt{DRAIN} matches the final large movement.  As with the row labels
(See Table~\ref{tab:intro-rowlabels}), the names are not keywords of the
language: the analyst invents them and gives each one a
...
so it is not available everywhere.  Chapter~\ref{chap:related} examines how
far each system implements it.

% ----------------------------------------------------------------
\section{Problem Statement}
\label{sec:intro-problem}
% ----------------------------------------------------------------
% a long code word in the first line has no break point
...
Support nevertheless remains uneven.\par\endgroup
...
Each one is stated so that the thesis can be judged against it.
...
\begin{enumerate}
  ...
  \item \textbf{Define the supported subset and verify its correctness.}
    ...
    second objective is to state the evaluated R010-style subset precisely: the
    main clauses, the output and skip modes, navigation and aggregate
    functions, and the pattern constructs from quantifiers and alternation to
    exclusions and \texttt{PERMUTE}.  It is then to establish that its results
...
resident server that keeps the table in its own managed memory: a Java
Virtual Machine (JVM) heap for Trino, or a System Global Area (SGA) for
Oracle.
...
35 cells.  Trino~473 completed 33.  Its complex nested cases at 160 and
...
parser, abstract syntax tree, semantic validation, logical plan, and the
design that connects
a declarative query to automata-based execution.

\textbf{Chapter~\ref{chap:implementation} --- Implementation and Optimization}
explains how a pattern is compiled into automata, how those automata run
over Pandas partitions, and how output and measures are built.  It also presents the performance layer: vectorized
...
paper describes its supported row-pattern features, its automaton-based
matching method,
...
A second manuscript extending this work has been submitted to a journal
