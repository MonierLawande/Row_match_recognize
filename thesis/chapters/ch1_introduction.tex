...
recognition into Pandas.  It connects relational pattern queries with
in-memory analytics and asks whether they can run without a database
server.  Three objectives guide the work.
...
\begin{enumerate}
  ...
  \item \textbf{Support and verify useful row-pattern semantics.}  The
    target is the evaluated R010-style subset.  It includes the evaluated
    forms of the main clauses, output and skip modes, and navigation
    and aggregate functions.  Its pattern constructs include
    quantifiers (\texttt{*}, \texttt{+},
...
At the time of evaluation, the regression suite contained 704 passing
...
Engine time grew almost linearly.  Its measured query-time memory was
higher than either database, but its process footprint was lower.
...
Across the four pattern families completed by all three systems,
Oracle's per-size mean time was $5.1$--$10.1\times$ the engine's.
Trino's was $11.4$--$13.8\times$ the engine's.
