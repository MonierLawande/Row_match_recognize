...
input and query logic under one CPU core and a 32\,GB memory limit.  The
...
cgroup-based metric.  Vendor-native counters have different scopes and
are not treated as equivalent measurements.
...
updated incrementally.  Unsupported expressions use the general
evaluator.  State-dependent conditions and some ambiguous pattern
...
227.9\,M rows.  The measured time and memory trends stayed close to
linear.  On the size averages, the engine remained
$5.1$--$10.1\times$ faster than Oracle~EE and
$11.4$--$13.8\times$ faster than Trino over the four pattern families
completed by all three systems.  Oracle~EE completed every
...
residency must therefore be read together.  The latest engine sweep and
...
automata construction, matching, and output assembly.  This separation
made each stage testable and gives future work clear extension points.


The optimization layer is also scoped.  It includes row-local condition
masks, compact DFA and run-length plans, compiled measures, fused output,
bounded caches, lazy row access, and conservative output projection.
The current partition loop is sequential.  The study therefore
evaluates a single-node \texttt{pandas} engine, not a distributed or
broadly parallel SQL system.  Trino and Oracle retain wider SQL
coverage, mature optimizers, and production execution infrastructure.

The central finding is scoped.  The supported row-pattern semantics can
be compiled and run directly over DataFrames.  The evaluated scenarios
agree with the reference systems, and performance scales predictably
over the measured ranges.
...
\texttt{pandas}~\ref{chap:evaluation}. &
...
\midrule
Feature support &
Clause, pattern, output modes, skip modes, navigation, and aggregate ~\ref{chap:methodology} and~\ref{chap:implementation}. &
...
limited by one workstation, two datasets, and the selected patterns.
The main benchmark uses twenty measured runs per cell; the larger
sweep uses five.  Its latest engine results and preserved database
results were not collected in one simultaneous rerun.  Trino and
...
explain output, match traces, and per-pattern counters would make each
execution path easier to verify.
