...
can run directly on Pandas DataFrames.  For the reported
correctness queries, the engine agreed with the applicable Trino~473,
Oracle, and hand-written Pandas baselines.
...
used 14--16 lines of detection logic, compared with 23--31 lines for
their hand-written Pandas versions.  Those functions also needed
...
combinations per system: five pattern families over six sizes from
100\,K to the full 2.22\,M rows, so 90 cells across the three systems.
Each cell is the mean of twenty measured runs after
five warmups and the stated $1.5\times$ IQR outlier rule.  Each system
used the same input and query logic under one CPU core and the same outer 32\,GB
memory limit.  Across those 30 cells the
proposed engine averaged 0.17\,s of execution time per cell and
6\,110\,392~rows/s of throughput.  Oracle averaged 1.04\,s and
Trino 1.98\,s per cell.  The proposed engine had the lowest time in all 30
...
14{,}992.90\,MB, respectively.  All six values use the same
instrument: the Linux control-group (cgroup) counter that reports the
memory of a set of processes.  But the process boundaries differ: one
engine process versus a complete database container.  Vendor-native
counters have different scopes and are not treated as equivalent
measurements.
...
residency must therefore be read together.  The engine values describe the
fixed benchmark revision recorded in the appendix (See
...
study therefore evaluates a single-node Pandas engine, not a
...
Every reported measurement has all of these optimized paths enabled,
each activating where it applies, so the results describe the combined
configuration.  Differential tests compare each eligible fast path with
the general path on the same input.  The evidence is therefore that
none of them changes the answer, not a measured contribution per
technique.  Two cases in Section~\ref{subsec:eval-ordering} do isolate
a path: the first forces the sort, and the second uses a predicate the
row-local path cannot compile.  A full ablation, each technique alone and in
combination, is left to future work.
...
The main claims of the thesis are listed with the evidence behind each and
the boundary within which it holds (See
Table~\ref{tab:conc-claim-validation}).  The purpose is to keep every
conclusion inside what the
evidence actually shows.

{\small
\begin{longtable}{@{}p{0.18\textwidth}p{0.27\textwidth}p{0.25\textwidth}p{0.20\textwidth}@{}}
\caption[Claims, evidence, and scope]{Each thesis claim, the evidence
behind it, and the scope within which it holds}
...
\midrule
\endfirsthead
\caption*{Table~\thetable{} (continued): Each thesis claim, the
evidence behind it, and the scope within which it holds} \\
\toprule
\textbf{Claim area} & \textbf{Supporting evidence} &
\textbf{Validated conclusion} & \textbf{Scope boundary} \\
\midrule
...
Pandas reported in Chapter~\ref{chap:evaluation}. &
...
execution concerns. &
The system remains a Pandas-centered prototype. \\
\end{longtable}
}

Three validity limits remain.  \emph{Internal validity}, whether the
measured differences come from the systems themselves, depends on the
reference outputs and their normalization.  \emph{Construct validity},
whether the chosen metrics stand for the claims made from them, rests
on row equality, time, throughput, memory, and line count.
\emph{External validity}, how far the results carry beyond this setup,
is limited by one workstation, two performance datasets, and the
selected patterns.
The main benchmark uses twenty measured runs per cell; the larger
sweep uses five.  Neither campaign was collected in one simultaneous
rerun.  Trino and
Oracle were not expert-tuned, and neither was the engine: all three ran
with their default settings under the stated limits.  Other
configurations may narrow the measured gaps.
...
Conditional and subquery ordering keys, vendor-specific extensions, and
other unsupported expression forms remain outside the validated scope.
...
scope.  This is a natural limit of an in-memory engine with its own
...
addressed that gap with a SQL-in-Pandas implementation that
...
scenarios agree with Trino, Oracle, and hand-written Pandas.
...
Python workflows where the data already lives in Pandas.
...
The path is Pandas-centered today.  A backend abstraction
...
state-dependent forms not covered by compiled plans.  A per-technique
ablation would also quantify what each optimized path contributes on
its own and alongside the others.  This remains a
