...
five warmups and the stated $1.5\times$ IQR outlier rule.  Each system used the same
...
14{,}992.90\,MB, respectively.  All six values use the same instrument, the Linux
control-group (cgroup) counter that reports the memory of a set of
processes, but the process boundaries differ: one engine process versus
a complete database container.  Vendor-native counters have different
...
output projection.  Every reported measurement has all of them
enabled, each activating where it applies, so the results describe the
combined configuration.  Differential tests compare each eligible fast path with the general
path on the same input, so the evidence is that none of them changes
the answer, not a measured contribution per technique.  Two sections do
isolate a path: Section~\ref{subsec:eval-ordering} forces the sort, and
...
combination, is left to future work.  Partition processing remains
sequential.  The
