#!/usr/bin/env python3
"""
Amazon UK Dataset Coverage Evaluation - Medium Sizes (25K-100K)

This script evaluates the match_recognize implementation for MEDIUM dataset sizes:
- 25,000 rows
- 35,000 rows  
- 50,000 rows
- 75,000 rows
...
Total: 5 sizes × 5 patterns = 25 tests
"""
...
def run_comprehensive_evaluation(dataset_sizes: List[int] = None):
    ...
    if dataset_sizes is None:
        dataset_sizes = [25000, 35000, 50000, 75000, 100000]
    
    print(f"\n{'#'*80}")
    print(f"🚀 MEDIUM SIZE EVALUATION - Amazon UK Dataset")
    ...
    # Generate summary
    print(f"\n\n{'='*80}")
    print(f"📊 EVALUATION SUMMARY - MEDIUM SIZES")
    print(f"{'='*80}")
    ...
    if successful_results:
        ...
        for pattern_name in patterns.keys():
            ...
            if pattern_results:
                ...
                print(f"   {pattern_name}: {avg_cov:.2f}% average")
    ...
    results_file = f"medium_sizes_results_{timestamp}.json"
...
def main():
    ...
    try:
        print(f"\n{'='*80}")
        print(f"🎯 MEDIUM SIZE EVALUATION (25K - 100K rows)")
        print(f"   Target sizes: 25K, 35K, 50K, 75K, 100K")
        print(f"   Total tests: 25 (5 sizes × 5 patterns)")
        print(f"{'='*80}\n")
